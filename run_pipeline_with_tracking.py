...
# Clean start: remove existing CSV if it exists
...
for frame_idx, (frame_name, bboxes) in enumerate(entries.items()):
    ...
    csv_lines = []  # Collect all lines for this frame
    for det_idx, det in enumerate(valid):
        ...
        rev_color, blink = revised.get(proj_id, (pred_color, False)) if proj_id is not None else (pred_color, False)
    ...
    # Write all CSV lines for this frame at once (more robust)
    if csv_lines:
        with open(results_path, 'a') as csv_file:
            csv_file.writelines(csv_lines)
        print(f"✅ {frame_name}: {len(csv_lines)} detections written to CSV")
    else:
        print(f"⚠️  {frame_name}: No valid detections found")
