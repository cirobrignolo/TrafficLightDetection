...
\section{Vista Panorámica de la Salida del Pipeline}
\label{sec:funcionamiento_panoramica}

Antes de entrar en el detalle técnico de cada etapa, conviene tener una imagen mental concreta de qué produce el módulo a lo largo del pipeline. Las Figuras~\ref{fig:pipeline_outputs_in} y~\ref{fig:pipeline_outputs_out} muestran, sobre un mismo \textit{frame} de entrada, la salida visualizada de las etapas que efectivamente producen un artefacto observable: la primera figura cubre la \textit{entrada y detección} (Etapas~1 y~2), y la segunda el \textit{reconocimiento y salida final} (Etapas~4 y~5). La Etapa~3 (asignación por algoritmo húngaro) no produce una salida visual propia: se limita a vincular las cajas de la Etapa~2 con las \textit{projection boxes} de la Etapa~1, y su efecto se observa indirectamente en el hecho de que cada caja del detector aparece en las etapas siguientes con un \texttt{signal\_id} concreto.

\begin{figure}[H]
    \centering
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{pipeline_01_projection_bbox.jpg}
        \caption[Etapa 1: \textit{projection box} (entrada del HD-Map).]{Etapa 1 — \textit{Projection box}: rectángulo en píxeles donde el HD-Map predice que aparece el semáforo (entrada al pipeline).}
        \label{fig:out_proj}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{pipeline_02_detection_output.jpg}
        \caption[Etapa 2: detección.]{Etapa 2 — Detección: \textit{bounding boxes} ajustados al semáforo real dentro de la \textit{projection box} expandida, sin identidad.}
        \label{fig:out_det}
    \end{subfigure}
    \caption[Salida del pipeline TLR: entrada y detección.]{Entrada y detección. La Etapa~1 entrega al pipeline la región donde se espera ver al semáforo (\textit{projection box} del HD-Map); la Etapa~2 produce \textit{bounding boxes} ajustados al semáforo real dentro de esa región. Las cajas todavía no llevan identidad: la Etapa~3 las asignará a cada \textit{projection box} en el paso siguiente.}
    \label{fig:pipeline_outputs_in}
\end{figure}

Una vez que cada detección quedó vinculada a una \textit{projection box} (Etapa~3, no visible), las dos etapas restantes asignan estado al semáforo y aplican las reglas temporales. La Figura~\ref{fig:pipeline_outputs_out} muestra esos dos pasos finales sobre el mismo \textit{frame}.

\begin{figure}[H]
    \centering
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{pipeline_03_recognition_output.jpg}
        \caption[Etapa 4: reconocimiento.]{Etapa 4 — Reconocimiento: cada \textit{bounding box} del detector recibe un color clasificado por el recognizer (\texttt{red}, \texttt{yellow}, \texttt{green} o \texttt{black}).}
        \label{fig:out_rec}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{pipeline_04_final_output.jpg}
        \caption[Etapa 5: \textit{tracking} (salida final).]{Etapa 5 — \textit{Tracking}: salida final del módulo con \texttt{signal\_id} persistente y color revisado tras aplicar las reglas temporales.}
        \label{fig:out_final}
    \end{subfigure}
    \caption[Salida del pipeline TLR: reconocimiento y final.]{Reconocimiento y salida final. La Etapa~4 clasifica el color de cada semáforo detectado; la Etapa~5 aplica las reglas temporales del \textit{tracker} (histéresis, regla asimétrica de seguridad) y devuelve el resultado final con el \texttt{signal\_id} persistente del HD-Map.}
    \label{fig:pipeline_outputs_out}
\end{figure}

Las secciones que siguen explican cada una de estas etapas en detalle: qué transformaciones aplica, qué parámetros usa y qué reglas de decisión rigen el comportamiento que se observa en las imágenes anteriores.
...
\hline
\end{tabularx}
\caption[Modelos pre-entrenados del TLR.]{Modelos pre-entrenados del TLR. La salida [B, R, Y, G] representa probabilidades de \textit{black}, \textit{red}, \textit{yellow}, \textit{green}. Los recognizers tienen pesos distintos pero la misma arquitectura general: 5 capas convolucionales con \textit{batch normalization}, \textit{pooling} y una capa \textit{fully-connected} final.}
\end{table}
...
\begin{align*}
\texttt{normalized} &= \texttt{pixel} - \texttt{means\_det}, \\
\texttt{means\_det} &= [102.98,\ 115.95,\ 122.77]
\end{align*}
...
\begin{equation}
\texttt{distance\_score} = \exp\!\left(-\tfrac{1}{2}\left(\tfrac{\Delta x}{\sigma_x}\right)^2 - \tfrac{1}{2}\left(\tfrac{\Delta y}{\sigma_y}\right)^2\right)
\end{equation}

donde $\Delta x = c_x^{\text{det}} - c_x^{\text{proj}}$ y $\Delta y = c_y^{\text{det}} - c_y^{\text{proj}}$ son las diferencias entre los centros de la detección y de la \texttt{projection\_roi}, en píxeles, y $\sigma_x = \sigma_y = 100.0$ define el ancho de la gaussiana.
...
\begin{align*}
\texttt{normalized} &= (\texttt{pixel} - \texttt{means\_rec}) \cdot \texttt{scale}, \\
\texttt{means\_rec} &= [66.56,\ 66.58,\ 69.06], \quad \texttt{scale} = 0.01
\end{align*}
...
Las Tablas~\ref{tab:parametros_pre} y~\ref{tab:parametros_post} agrupan todos los parámetros numéricos del módulo, con su archivo fuente y propósito. La división responde a las dos mitades del pipeline: la primera tabla cubre las etapas de \textit{entrada y selección} (preprocesamiento, detección y asignación); la segunda cubre las etapas de \textit{decisión sobre el semáforo} (reconocimiento y \textit{tracking}).
...
\hline
\texttt{query\_radius} & 150\,m & \texttt{region\_proposal.cc} & Radio de query al HD-Map \\
\texttt{crop\_scale} & 2.5 & \texttt{cropbox.cc} & Expansión de la \texttt{projection\_roi} \\
\texttt{min\_crop\_size} & 270\,px & \texttt{cropbox.cc} & Tamaño mínimo del \textit{crop} \\
\texttt{means\_det} BGR & [102.98, 115.95, 122.77] & \texttt{detection.cc} & Medias para normalización del detector \\
\hline
\multicolumn{4}{|l|}{\textit{Detección}} \\
\hline
\texttt{input\_size} & 270$\times$270 & \texttt{detection.cc} & Resolución de entrada al detector \\
\texttt{nms\_rpn} & 0.7 & RPN interno & IoU \textit{threshold} en RPN \\
\texttt{nms\_rcnn} & 0.5 & RCNN interno & IoU \textit{threshold} en RCNN \\
\texttt{nms\_global} & 0.6 & \texttt{detection.cc:381} & IoU \textit{threshold} global \\
\hline
\multicolumn{4}{|l|}{\textit{Asignación}} \\
\hline
\texttt{distance\_weight} & 0.7 & \texttt{select.cc} & Peso de distancia en costo combinado \\
\texttt{confidence\_weight} & 0.3 & \texttt{select.cc} & Peso de confianza en costo combinado \\
\texttt{gaussian\_sigma} & 100.0 & \texttt{select.cc} & $\sigma$ del \textit{score} gaussiano 2D \\
\texttt{max\_detection\_score} & 0.9 & \texttt{select.cc} & \textit{Clip} máximo del \textit{score} \\
\hline
\end{tabularx}
\caption{Parámetros de las etapas de entrada y selección del módulo TLR (preprocesamiento, detección y asignación).}
\label{tab:parametros_pre}
\end{table}
...
\hline
\texttt{vert\_shape} & 96$\times$32 & \texttt{classify.cc} & Input del recognizer vertical \\
\texttt{hori\_shape} & 32$\times$96 & \texttt{classify.cc} & Input del recognizer horizontal \\
\texttt{quad\_shape} & 64$\times$64 & \texttt{classify.cc} & Input del recognizer \textit{quad} \\
\texttt{means\_rec} BGR & [66.56, 66.58, 69.06] & \texttt{classify.cc} & Medias para normalización de recognizers \\
\texttt{scale\_factor} & 0.01 & \texttt{classify.cc} & Escala post-normalización \\
\texttt{classify\_threshold} & 0.5 & \texttt{recognition.cc:48} & Umbral en \texttt{Prob2Color} \\
\hline
\multicolumn{4}{|l|}{\textit{Tracking}} \\
\hline
\texttt{revise\_time\_s} & 1.5\,s & \texttt{semantic\_decision.cc} & Ventana temporal de validación \\
\texttt{blink\_threshold\_s} & 0.55\,s & \texttt{semantic\_decision.cc} & Umbral para detectar \textit{blink} \\
\texttt{hysteretic\_threshold} & 1 & \texttt{semantic\_decision.cc} & Frames para aceptar transición a BLACK \\
\hline
\end{tabularx}
\caption{Parámetros de las etapas de decisión del módulo TLR (reconocimiento y \textit{tracking}).}
\label{tab:parametros_post}
\end{table}
