...
\begin{tikzpicture}[
    ...
    \draw[arrow] (trk) -- (out);
\end{tikzpicture}
\caption{Pipeline del módulo TLR de Apollo: cinco etapas secuenciales que transforman un \textit{frame} de cámara más información geográfica en un conjunto de estados de semáforos identificados.}
...
\caption{Diagrama de secuencia del pipeline TLR: cada flecha representa el dato que cruza de una etapa a la siguiente. Las cantidades $M$ (cuántos semáforos hay en el HD-Map a esa distancia), $N$ (cuántas detecciones produjo la red) y $K \le \min(M,N)$ (cuántas asignaciones encontró el húngaro) varían por \textit{frame}.}
